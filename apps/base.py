...
class BaseApp(hass.Hass):
    ...
    def get_seconds_until_night_end(self):
        ...
        night_end_datetime = datetime.combine(now.date(), night_end_time, tzinfo=timezone.utc)
    ...
    def above_max_illumination(self, value = None):
        ...
        """
        if isinstance(value, int) or isinstance(value, float):
            # For direct numeric inputs, consider above max when value > _max_illumination
            if value > self._max_illumination:
                return True
            return False
