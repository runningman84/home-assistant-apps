...
class LightSwitch(hass.Hass):
    ...
    def remote_callback(self, event_name, data, kwargs):
        ...
        self.log("Callback light_button_callback from event {}:{}".format(entity_id, event_type))
        ...
        if event_type == "brightness_up_click":
            ...
            for light in self.__lights:
                self.turn_on(
                                    entity_id=light,
                                    color_temp_kelvin=self.get_state(light, attribute = "min_color_temp_kelvin"))
            ...
            for light in self.__lights:
                self.turn_on(
                                    entity_id=light,
                                    color_temp_kelvin=self.get_state(light, attribute = "max_color_temp_kelvin"))
